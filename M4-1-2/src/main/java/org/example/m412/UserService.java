...
public class UserService {
	...
	public static JSONObject getUserInformation(String handle) throws IOException {
		// 查询数据库
		try {
			User.UserInfo userInfo = DatabaseService.getUserInfo(handle);
			// 数据库里有信息
			if (userInfo != null) {
				// 检查信息有效性
				LocalDateTime lastUpdated = LocalDateTime.parse(userInfo.getUpdatedAt(),
						DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
				LocalDateTime now = LocalDateTime.now();
				// 在有效期内
				if (Duration.between(lastUpdated, now).getSeconds() <= 30) {
					JSONObject jsonResponse = new JSONObject();
					jsonResponse.put("handle", userInfo.getHandle());
					jsonResponse.put("rating", userInfo.getRating());
					jsonResponse.put("rank", userInfo.getRank());
					jsonResponse.put("updated_at", userInfo.getUpdatedAt());
					jsonResponse.put(handle, jsonResponse);
					return jsonResponse;
				}
			}
		} catch (Exception e) {
		...
		if (apiData.getBoolean("success")) {
			try {
				...
				apiData.put("updated_at", updatedAt);
			...
			if (ratings != null && !ratings.isEmpty()) {
				...
				if ((currentTime - lastUpdatedTime) <= 30 * 1000) {
					...
					for (User.UserRating rating : ratings) {
						...
						ratingData.put("contest_id", rating.getContestId());
						ratingData.put("contest_name", rating.getContestName());
						ratingData.put("rank", rating.getRank());
						ratingData.put("old_rating", rating.getOldRating());
						ratingData.put("new_rating", rating.getNewRating());
						ratingData.put("rating_updated_at", rating.getRatingUpdatedAt()); // String
						ratingData.put("updated_at", rating.getUpdatedAt()); // String
		...
		if (apiData != null) {
			try {
				for (int i = 0; i < apiData.length(); i++) {
					...
					rating.put("rating_updated_at", ratingUpdatedAt);
					rating.put("updated_at", updatedAt);
				}
				return apiData;
			} catch (Exception e) {
